...
const FootePanel = ({user}: any) => {
  ...
  return (
    <div className="flex flex-row w-full items-center justify-between h-[7%] border-t px-[5px] bg-[#B5BFC6] dark:bg-[#24273D] border-[white] dark:border-[#323657]">
        {userPhoto && <img onClick={goToProfile} src={userPhoto} className='w-[40px] h-[40px] rounded-[20px] cursor-pointer'/>}
        {!user?.photoURL && <Image onClick={goToProfile} alt='' src={defAvatar} className='w-[40px] h-[40px] rounded-[20px] cursor-pointer'/>}
        <Image onClick={goToMain} className='w-[40px] h-[40px] cursor-pointer' src={chats} alt='icon'/>
        <Image onClick={goToSettings}  className='w-[40px] h-[40px] cursor-pointer' src={settings} alt='icon'/>
        <Image onClick={signOutFromAccount} className='w-[40px] h-[40px] cursor-pointer' src={exitDoor} alt='icon'/>
    </div>
  )
}
