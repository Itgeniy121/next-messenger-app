...
const AuthForm = () => {
    ...
    const onSubmit: SubmitHandler<authTypes> = async (data, e) => {
        ...
        if (password === confirmPassword) {
            await createNewUser(email, password, name).then(
                async (user: any) => {
                    if (user !== undefined && photoURL) {
                        await uploadProfilePhoto(photoURL, name, user.uid)
                            .then(async () => {
                                await setTimeout(() => {
                                    router.push('/');
                                }, 1500);
    ...
    return (
        <form
            ...
            className="flex flex-col justify-center items-start w-full mt-[20px]"
        >
            ...
            <div>
                <div className="flex flex-col mt-[15px]">
                    <div className="flex flex-row">
                        ...
                        />
                        &nbsp;&nbsp;
                        <p className="text-[12px] text-[#9495A4] font-[200] extramin:text-[15px]">
                            Это мессенджер для панков, тут правил нет!
                        </p>
                    </div>
                    {errors?.check && (
                        <div className="text-[red] text-[11px] ml-[20px]">
                ...
                />
                {img ? (
                    <p className="text-[15px] font-[200]">{img.name}</p>
                ) : (
                    <label
                        className="flex flex-row justify-start items-center w-[200px] mt-[20px] cursor-pointer"
                        htmlFor="file"
                    >
                        <Image
                            src={upload}
            ...
            <button className="flex flex-row justify-center items-center bg-[#EE3A57] w-[100%] py-[12px] rounded-[3px] mt-[15px] hover:bg-[#c74057]">
                <p className="text-[16px] !font-[200] mr-[10px]">Создать</p>
            </button>
