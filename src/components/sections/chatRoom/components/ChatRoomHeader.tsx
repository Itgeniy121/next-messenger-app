      ...
      <div className="flex flex-row justify-center items-center">
        <Link href="/">
          ...
          <Image
            ...
            width={100}
            height={100}
            className="w-[30px] mr-[20px] flex sidebar:hidden"
          />
        </Link>
        {user && user !== undefined && user[0]?.photoUrl && (
          <Image
            onClick={goToProfile}
            src={user[0]?.photoUrl}
            width={100}
            height={100}
            alt="avatar"
            className="size-[30px] sidebar:size-[45px] rounded-[50%] cursor-pointer"
          />
        )}
        {user && user !== undefined && !user[0]?.photoUrl && (
          <Image
        ...
        <div className="flex flex-col justify-center items-start h-full ml-[10px]">
          <p className="text-[15px] text-[white] font-[300]">
            {user && user !== undefined && user[0]?.name}
          </p>
          <p className="text-[12px] text-[#9495A4] font-[300]">
